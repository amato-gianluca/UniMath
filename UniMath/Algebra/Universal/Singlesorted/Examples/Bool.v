...
Require Import UniMath.Algebra.Universal.Singlesorted.VTerms.

Local Open Scope stn.

Definition bool_signature := make_signature_simple [0; 0; 1; 2; 2; 2].

(** ** Algebra structure over type bool. *)

Definition bool_algebra := make_algebra_simple'
  bool_signature
  boolset
...
Definition T := gterm bool_signature.
...
Definition T := term bool_signature natset.
...
Definition interp (α: natset → bool_algebra) (t: T): bool :=
  fromterm (ops bool_algebra) α t.

(** Computations and interactive proofs. *)

Module Tests.

Definition x : T := varterm (0: natset).
Definition y : T := varterm (1: natset).
Definition z : T := varterm (2: natset).
