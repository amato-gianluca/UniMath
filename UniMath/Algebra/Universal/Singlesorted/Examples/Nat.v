...
Definition nat_signature: signature_simple := make_signature_simple [ 0 ; 1 ].

Definition nat_zero_op: names nat_signature := ●0.
Definition nat_succ_op: names nat_signature := ●1.

Definition nat_algebra := make_algebra_simple' nat_signature natset [( 0 ; S )].
...
Definition z2_algebra := make_algebra_simple' nat_signature boolset [( false ; negb )].

Definition nat_to_z2 : nat_algebra → z2_algebra
  := nat_rect (λ _, bool) false (λ n HP, negb HP).

Goal nat_to_z2 0 = false.
Proof. apply idpath. Defined.

Goal nat_to_z2 1 = true.
Proof. apply idpath. Defined.

Goal nat_to_z2 2 = false.
Proof. apply idpath. Defined.
...
Definition nat2term (n: nat): gterm nat_signature
  := nat_rect
       (λ _, gterm nat_signature)
       nat_zero
       (λ (n: nat) (tn: gterm nat_signature), nat_succ tn)
       n.
