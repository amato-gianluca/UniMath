...
Require Import UniMath.Algebra.Universal.Singlesorted.Examples.Nat.
Require Import UniMath.Algebra.Universal.Singlesorted.Examples.Bool.
...
Section NatLowLevel.
  ...
  Goal Terms.opexec nat_succ_op (just 1) = just 1.
  Proof. apply idpath. Qed.

  Goal Terms.opexec nat_succ_op (just 0) = nothing.
  Proof. apply idpath. Qed.
  ...
  Goal @Terms.oplistexec nat_signature [] = just 0.
  Proof. apply idpath. Qed.

  Goal Terms.oplistexec one_oplist = just 1.
  Proof. apply idpath. Qed.

  Goal Terms.oplistexec zero_one_oplist = just 2.
  Proof. apply idpath. Qed.
  ...
  Goal Terms.stackconcatenate (just 1) (just 2) = just  3.
  Proof. apply idpath. Qed.

  Local Definition one_term : gterm nat_signature := make_term(l:=one_oplist) (idpath _).

  Local Definition zero_term : gterm nat_signature := make_term(l:=zero_oplist) (idpath _).
  ...
  Goal vec_map term2oplist (pr1 (Terms.oplist2vecoplist zero_one_oplist (idpath _))) = vcons zero_oplist (vcons one_oplist vnil).
  Proof. apply idpath. Qed.
...
Section Nat.
  ...
  Goal geval nat_algebra term_zero = 0.
  Proof. apply idpath. Qed.

  Goal geval nat_algebra term_one = 1.
  Proof. apply idpath. Qed.

  Goal geval nat_algebra (nat2term 4) = 4.
  Proof. apply idpath. Qed.
...
Section NatHom.
  ...
  Local Definition nat_algebra2
    := make_algebra_simple nat_signature natset [( λ _, 1 ;  λ x, S (pr1 x) )].
...
Section Bool.
  Import UniMath.Algebra.Universal.Singlesorted.Examples.Bool.GTerm.
  Local Open Scope stn.
  ...
  *)
  ...
  Local Lemma l1: geval bool_algebra  top = true.
  Proof. apply idpath. Defined.

  Local Lemma l2: geval bool_algebra (neg top) = false.
  Proof. apply idpath. Defined.

  (* USED TO WORK

  Local Lemma l3: geval bool_algebra (conj top bot) = false.
  Proof. apply idpath. Defined.

  Local Lemma l4: geval bool_algebra simple_t = false.
  Proof. apply idpath. Defined.
  *)
